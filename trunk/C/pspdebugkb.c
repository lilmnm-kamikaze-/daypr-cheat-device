...
#include <stdio.h>
#include <string.h>
#include "H/pspdebugkb.h"
#include "H/snprintf.h"
#include "H/Defines.h"
...
static char *commandRow[] = { "Shift", "[SPACE]", "Back", "Clear", "Done" };
...
void pspDebugKbDrawKey(int row, int col, int highlight) {
  ...
  if (highlight) {
    TextColor(PSP_DEBUG_KB_CHAR_HIGHLIGHT);
    BackGround(PSP_DEBUG_KB_BACK_HIGHLIGHT);
  } else {
    TextColor(PSP_DEBUG_KB_CHAR_COLOUR);
    BackGround(PSP_DEBUG_KB_BACK_COLOUR);
  }
  ...
  if (row == PSP_DEBUG_KB_COMMAND_ROW) {
    ...
    SetXY(PSP_DEBUG_KB_BOX_X + (spacing * (col + 1)) + charsTo, PSP_DEBUG_KB_BOX_Y + (PSP_DEBUG_KB_SPACING_Y * (row + 2)));
    pspDebugScreenPrintf("%s", commandRow[col]);
  } else {
    SetXY(PSP_DEBUG_KB_BOX_X + PSP_DEBUG_KB_OFFSET_X + (PSP_DEBUG_KB_SPACING_X * col), PSP_DEBUG_KB_BOX_Y + (PSP_DEBUG_KB_SPACING_Y * (row + 2)));
...
void pspDebugKbDrawBox() {
  ...
  SetXY(PSP_DEBUG_KB_BOX_X, PSP_DEBUG_KB_BOX_Y);
  pspDebugScreenPrintf("+");
  SetXY(PSP_DEBUG_KB_BOX_X, PSP_DEBUG_KB_BOX_Y + PSP_DEBUG_KB_BOX_HEIGHT);
  pspDebugScreenPrintf("+");
  SetXY(PSP_DEBUG_KB_BOX_X + PSP_DEBUG_KB_BOX_WIDTH, PSP_DEBUG_KB_BOX_Y);
  pspDebugScreenPrintf("+");
  SetXY(PSP_DEBUG_KB_BOX_X + PSP_DEBUG_KB_BOX_WIDTH, PSP_DEBUG_KB_BOX_Y + PSP_DEBUG_KB_BOX_HEIGHT);
  pspDebugScreenPrintf("+");

  for (i = 1; i < PSP_DEBUG_KB_BOX_WIDTH; i++) {
    SetXY(PSP_DEBUG_KB_BOX_X + i, PSP_DEBUG_KB_BOX_Y);
    pspDebugScreenPrintf("-");
    SetXY(PSP_DEBUG_KB_BOX_X + i, PSP_DEBUG_KB_BOX_Y + PSP_DEBUG_KB_BOX_HEIGHT);
    pspDebugScreenPrintf("-");
  }

  for (i = 1; i < PSP_DEBUG_KB_BOX_HEIGHT; i++) {
    SetXY(PSP_DEBUG_KB_BOX_X, PSP_DEBUG_KB_BOX_Y + i);
    pspDebugScreenPrintf("|");
    SetXY(PSP_DEBUG_KB_BOX_X + PSP_DEBUG_KB_BOX_WIDTH, PSP_DEBUG_KB_BOX_Y + i);
    pspDebugScreenPrintf("|");
  }
...
void pspDebugKbDrawString(char* str) {
  ...
  SetXY(PSP_DEBUG_KB_BOX_X + ((PSP_DEBUG_KB_BOX_WIDTH - PSP_DEBUG_KB_MAXLEN) / 2), PSP_DEBUG_KB_BOX_Y + 1);
